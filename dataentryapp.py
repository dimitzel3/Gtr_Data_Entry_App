...
def insert_record(
    ...
    conv_da_refs,
    sheep_bio_kg,
    sheep_bio_kg_scale,     # NEW
    goat_bio_kg,
    goat_bio_kg_scale,      # NEW
    bio_da_refs,
    total_all_da,
    total_all_scale,
    diff_da_vs_scale,
    ...
    data = {
        ...
        "sheep_bio_kg_scale": to_float_or_none(sheep_bio_kg_scale),  # NEW
        "goat_bio_kg": to_float_or_none(goat_bio_kg),
        "goat_bio_kg_scale": to_float_or_none(goat_bio_kg_scale),    # NEW
...
def get_all_records() -> pd.DataFrame:
    res = supabase.table("routes").select("*").order("id", desc=True).execute()
    data = res.data or []
...
def update_record(
    ...
    conv_da_refs=None,
    sheep_bio_kg=None,
    sheep_bio_kg_scale=None,  # NEW
    goat_bio_kg=None,
    goat_bio_kg_scale=None,   # NEW
    bio_da_refs=None,
    total_all_da=None,
    ...
    if customer is not None:
        update_data["customer"] = customer  # NEW
    ...
    if sheep_bio_kg_scale is not None:
        update_data["sheep_bio_kg_scale"] = to_float_or_none(sheep_bio_kg_scale)  # NEW
    if goat_bio_kg is not None:
        update_data["goat_bio_kg"] = to_float_or_none(goat_bio_kg)
    if goat_bio_kg_scale is not None:
        update_data["goat_bio_kg_scale"] = to_float_or_none(goat_bio_kg_scale)    # NEW
...
def close_record(record_id):
    ...
    supabase.table("routes").update({"is_closed": True, "closed_at": now}).eq("id", record_id).execute()
...
with tab_new:
    ...
    with st.form("route_form", clear_on_submit=True):
        ...
        # NEW: Πελάτης κάτω από το δρομολόγιο
        customer = st.selectbox("Πελάτης", CUSTOMER_OPTIONS)
        ...
        with col_m3:
            ...
            # NEW: κάτω από το αντίστοιχο κιλά
            sheep_bio_kg_scale = st.number_input("ΠρόΒΕΙΟ ΒΙΟΛΟΓΙΚΟ ΓΑΛΑ (ΖΥΓΟΛΟΓΙΟ)", min_value=0.0, step=1.0, format="%.2f")
        with col_m4:
            goat_bio_kg = st.number_input("Γίδινο Βιολογικό Γάλα (κιλά)", min_value=0.0, step=1.0, format="%.2f")
            # NEW: κάτω από το αντίστοιχο κιλά
            goat_bio_kg_scale = st.number_input("ΓΙΔΙΝΟ ΒΙΟΛΟΓΙΚΟ ΓΑΛΑ (ΖΥΓΟΛΟΓΙΟ)", min_value=0.0, step=1.0, format="%.2f")
        ...
        if submitted:
            ...
            try:
                insert_record(
                    ...
                    sheep_bio_kg_scale=sheep_bio_kg_scale,
                    goat_bio_kg=goat_bio_kg,
                    goat_bio_kg_scale=goat_bio_kg_scale,
...
with tab_open:
    ...
    if open_df is not None and not open_df.empty:
        ...
        if "dt" in filtered_open.columns and start_date and end_date:
            filtered_open = filtered_open[(filtered_open["dt"] >= start_date) & (filtered_open["dt"] <= end_date)]

        st.markdown("### 📋 Ανοιχτά δρομολόγια")
        show_cols = [
            "id", "dt", "started_at_dt", "route", "customer", "plate",
        ...
        if not filtered_open.empty:
            ...
            for _, r in filtered_open.iterrows():
                ...
                id_labels.append(f"ID {r['id']} – {r['plate']} – {r['driver']} – {start_str}")

            selected_label = st.selectbox("Διάλεξε δρομολόγιο", options=id_labels, index=0)
                ...
                with st.form(f"edit_form_{selected_id}"):
                    ...
                    with ecol2:
                        e_customer = st.selectbox(
                            "Πελάτης",
                            CUSTOMER_OPTIONS,
                            index=CUSTOMER_OPTIONS.index(row["customer"]) if row.get("customer") in CUSTOMER_OPTIONS else 0,
                            key=f"customer_{selected_id}",
                        )
                    with ecol3:
                    ...
                    with em3:
                        ...
                        e_sheep_bio_kg_scale = st.number_input(
                            "ΠΡΟΒΕΙΟ ΒΙΟΛΟΓΙΚΟ ΓΑΛΑ (ΖΥΓΟΛΟΓΙΟ)",
                            min_value=0.0, step=1.0, format="%.2f",
                            value=float(row.get("sheep_bio_kg_scale") or 0),
                    ...
                    with em4:
                        ...
                        e_goat_bio_kg_scale = st.number_input(
                            "ΓΙΔΙΝΟ ΒΙΟΛΟΓΙΚΟ ΓΑΛΑ (ΖΥΓΟΛΟΓΙΟ)",
                            min_value=0.0, step=1.0, format="%.2f",
                            value=float(row.get("goat_bio_kg_scale") or 0),
                        ...
                        if validate_km():
                            try:
                                update_record(
                                    ...
                                    sheep_bio_kg_scale=e_sheep_bio_kg_scale,
                                    goat_bio_kg=e_goat_bio_kg,
                                    goat_bio_kg_scale=e_goat_bio_kg_scale,
...
with tab_all:
    ...
    if df is not None and not df.empty:
        ...
        df["started_at_dt"] = pd.to_datetime(df["started_at"], errors="coerce") if "started_at" in df.columns else pd.NaT
        df["closed_at_dt"] = pd.to_datetime(df["closed_at"], errors="coerce") if "closed_at" in df.columns else pd.NaT
        ...
        if start_date and end_date:
            filtered_df = filtered_df[(filtered_df["dt"] >= start_date) & (filtered_df["dt"] <= end_date)]

        st.markdown("### 📊 Αποτελέσματα")
        show_cols = [
            "id", "dt", "started_at_dt", "closed_at_dt",
            "route", "customer", "plate", "start_km", "end_km", "total_km",
            "driver",
            "sheep_conv_kg", "goat_conv_kg", "total_conv_kg_scale",
            "conv_da_refs",
            "sheep_bio_kg", "sheep_bio_kg_scale",
            "goat_bio_kg", "goat_bio_kg_scale",
            "bio_da_refs",
        ...
        if not filtered_df.empty:
            df_export = filtered_df.copy().astype(str)
            output = io.BytesIO()
        ...
        if not filtered_df.empty:
            for _, r in filtered_df.iterrows():
                ...
                with colA:
                    ...
                    if pd.notna(r.get("started_at_dt")):
                        start_str = r["started_at_dt"].strftime("%d/%m %H:%M")
                    st.write(f"ID: {r['id']} – {r['plate']} – {r.get('driver', '')} – {start_str}")
                with colB:
                    if st.button("🗑️", key=f"del_{r['id']}"):
                        try:
                            delete_record(int(r["id"]))
                            st.success(f"Η εγγραφή με ID {r['id']} διαγράφηκε.")
                            st.experimental_rerun()
                        except Exception as e:
